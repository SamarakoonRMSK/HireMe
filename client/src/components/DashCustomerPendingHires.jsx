...
export default function DashCustomerPendingHires() {
  ...
  useEffect(() => {
    const fetchPendingHires = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/hire/customer-pending-hires/${currentUser._id}`);
        const data = await res.json();

        if (!res.ok) {
          throw new Error(data.message || 'Failed to fetch pending hires');
        }
        setHires(data);
        setError(null);
      } catch (err) {
  ...
  const handleCancelHire = async (hireId) => {
    try {
      const res = await fetch(`/api/hire/cancel/${hireId}/${currentUser._id}`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        setError(data.message || 'Failed to cancel hire');
        return;
      }
      setHires(hires.filter((hire) => hire._id !== hireId));
      setError(null);
    } catch (err) {
      setError('Failed to cancel hire. Please try again.');
    }
  };
            ...
            <Card key={hire._id} className="border border-gray-200 shadow-sm w-full">
              ...
              <div className="flex gap-3 mt-4">
                <Button
                  ...
                  size="sm"
                  outline
                  onClick={() => handleCancelHire(hire._id)}
                >
                  Cancel Hire
                </Button>
              </div>
